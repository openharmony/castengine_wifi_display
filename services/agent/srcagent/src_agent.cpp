...
SrcAgent::SrcAgent() : Agent(AgentType::SRC_AGENT)
{
...
void SrcAgent::OnSessionNotify(SessionStatusMsg::Ptr &statusMsg)
{
    SHARING_LOGD("trace.");
    RETURN_IF_NULL(statusMsg);
    SHARING_LOGI("notify status: %{public}s, agentId: %{public}u.",
    ...
    switch (statusMsg->status) {
        case SessionNotifyStatus::NOTIFY_PROSUMER_CREATE:
            NotifyProducer(statusMsg, EventType::EVENT_MEDIA_PRODUCER_CREATE);
            break;
        case SessionNotifyStatus::STATE_SESSION_STARTED:
            NotifyProducer(statusMsg, EventType::EVENT_MEDIA_PRODUCER_START);
            break;
        case SessionNotifyStatus::STATE_SESSION_STOPED:
            NotifyProducer(statusMsg, EventType::EVENT_MEDIA_PRODUCER_STOP);
            break;
        case SessionNotifyStatus::STATE_SESSION_PAUSED:
            NotifyProducer(statusMsg, EventType::EVENT_MEDIA_PRODUCER_PAUSE);
            break;
        case SessionNotifyStatus::STATE_SESSION_RESUMED:
            NotifyProducer(statusMsg, EventType::EVENT_MEDIA_PRODUCER_RESUME);
            break;
        case SessionNotifyStatus::STATE_SESSION_DESTROYED:
            NotifyProducer(statusMsg, EventType::EVENT_MEDIA_PRODUCER_DESTROY);
            break;
...
void SrcAgent::NotifyProducer(SessionStatusMsg::Ptr &statusMsg, EventType type)
{
    SHARING_LOGD("trace.");
    RETURN_IF_NULL(statusMsg);
